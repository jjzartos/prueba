...
public class InternalFrameCalendario extends JInternalFrame {
	...
	public InternalFrameCalendario() {
    ...
    public void printCalendar(int[][] layout) {
        int l = 0;
        int d = 0 ;
        System.out.println("                                "+(day.getYear()-1));
        for ( int i = 0 ; i < layout.length ; i++ ) {
            for ( int j = 0; j < layout[i].length ; j++ ) {
                if ( i % 6 == 0 && j == 0) {
                    System.out.printf("%n%s%n%s%n", mh[l++],daysHeader);
                    //JOptionPane.showMessageDialog(null, "                                "+(day.getYear()-1)+"\n"+"%n%s%n%s%n"+ mh[l++]+daysHeader+"\n"+" "+"\n"+"%3s"+ String.valueOf(d == 0 ? " ": d+""));
                }
                if ( j > 0 &&  j % 7  == 0 ){
                    System.out.print(" ");
                }
