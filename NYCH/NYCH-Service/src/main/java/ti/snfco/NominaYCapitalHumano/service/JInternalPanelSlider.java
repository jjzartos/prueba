...
public class JInternalPanelSlider<ParentType extends JInternalFrame> {
	...
	public JInternalPanelSlider(ParentType parent) {
		if (parent == null) {
		...
		if (useSlideButton) {
			...
			statusPanel.add(new JButton("Deslizar Derecha") {
				{
					setMargin(new Insets(0, 0, 0, 0));
					setBounds(0, 0,130, 22);
				}
	...
	private void attach() {
		...
		//para abrir varios JInternalFrame
		if (w instanceof JInternalFrame) {
			final JInternalFrame j = (JInternalFrame) w;
	...
	private void enableUserInput(final ParentType w) {
		if (w instanceof JInternalFrame) {
			((JInternalFrame) w).getGlassPane().setVisible(false);
		}
	}

	private void disableUserInput(final ParentType w) {
		if (w instanceof JInternalFrame) {
			((JInternalFrame) w).setGlassPane(glassPane);
		}
	...
	private void enableTransparentOverylay() {
		if (parent instanceof JInternalFrame) {
			((JInternalFrame) parent).getContentPane().setBackground(jPanels.get(0).getBackground());
